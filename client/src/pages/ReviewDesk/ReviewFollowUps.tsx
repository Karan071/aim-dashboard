...
import { FollowUpsReview, coachesList } from "@/data/Data";
...
function FollowUpsTable() {
    const [usersData, setUsersData] = useState(FollowUpsReview);
    ...
    const handleRowClick = (user: (typeof FollowUpsReview)[0]) => {
    ...
    const handleAssignUsers = (userId: string) => {
      ...
      if (user && user.ApprovedBy) {
        setSelectedAssignees(user.ApprovedBy.map(approver => approver.name));
      } else {
    ...
    const AssignmentModal = () => {
      ...
      const currentUserAssignedImages = currentUser?.ApprovedBy || [];
      ...
      const availableAssignees = coachesList.map(coach => {
        ...
        const currentAssignment = currentUserAssignedImages.find(approver => approver.name === coach.name);
            ...
            <Table className="w-full caption-top border-collapse overflow-y-visible">
              <TableHeader className="bg-[var(--faded)] hover:bg-[var(--faded)] dark:bg-[var(--faded)] opacity-100">
                <TableRow>
                  ...
                  <TableHead
                    onClick={() => requestSort("DateApproved")}
                    className="cursor-pointer text-[var(--text)]"
                  >
                    Date Approved{" "}
                    {sortConfig?.key === "DateApproved" &&
                      (sortConfig.direction === "ascending" ? "↑" : "↓")}
                  </TableHead>
                  <TableHead
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className="flex -space-x-2">
                                                   {user.ApprovedBy?.map((approver, index) => (
                             <div
                               key={index}
                               className="h-8 w-8 rounded-full overflow-hidden border-2 border-white shadow-sm"
                               title={approver.name}
                    ...
                    <TableCell>{user.DateApproved}</TableCell>
                    <TableCell>{user.View}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
