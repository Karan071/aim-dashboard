...
import { FormsDataReview,} from "@/data/Data";
...
function FormTable() {
    ...
    const [selectedStack, setSelectedStack] = useState<
        typeof FormsDataReview
    >(FormsDataReview[0] ? [FormsDataReview[0]] : []);
    const [focusedId, setFocusedId] = useState<number | null>(FormsDataReview[0]?.id || null);

    // Sorting logic
    const sortedData = [...FormsDataReview];
    if (sortConfig !== null) {
        sortedData.sort((a, b) => {
    ...
    const handleRowClick = (user: (typeof FormsDataReview)[0]) => {
