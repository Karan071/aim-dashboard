...
import {  ReportTable } from "@/data/Data";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--text-head)]">Reports</h1>
        <StatsCards />
        <Buttonbar/>
        <Button
                  variant="border"
    ...
    <div className="flex justify-between px-4 py-3 bg-[var(--background)] rounded-sm gap-4 border flex-wrap shadow-none">
      ...
      <div className="flex gap-4">
        <Button variant="standard" size="new">
          ...
          <span className="">Email to Stakeholder</span>
        </Button>
        <Button variant="standard" size="new">
          <Clock className="h-3 w-3" />
          <span className="">Schedule Future Report</span>
        </Button>
        <Button variant="standard" size="new">
          <File className="h-3 w-3" />
          <span className="">View Linked Records</span>
        </Button>
      </div>
    </div>
  );
...
function AdvancedFilters({ onClose }: FilterProps) {
  ...
  const [holder, setHolder] = useState("Coach");
  const [product, setProduct] = useState("Session");
      ...
      >
        ...
        <div className="flex ">
          ...
          <div className="p-6 overflow-y-auto relative w-full">
            {activeTab === "Search" && (
                          <>
                            <label htmlFor="Gen" className="text-[var(--text)]">Input by Name, Email, ID, Code :</label>
...
function TableSection() {
  ...
  const [selectedStack, setSelectedStack] = useState<
    typeof ReportTable
  >(ReportTable[0] ? [ReportTable[0]] : []);
  const [focusedId, setFocusedId] = useState<number | null>(ReportTable[0]?.id || null);

  // Sorting logic
  const sortedData = [...ReportTable];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
  ...
  */
  ...
  const handleRowClick = (user: (typeof ReportTable)[0]) => {
          ...
          <Table className="w-full caption-top border-collapse overflow-y-visible">
            <TableHeader className="bg-[var(--faded)] hover:bg-[var(--faded)] dark:bg-[var(--faded)] opacity-100">
              <TableRow>
                ...
                <TableHead
                  onClick={() => requestSort("ReportName")}
            ...
            <TableBody className="overflow-visible relative z-0">
              {currentRecords.map((user) => (
                ...
                >
                  ...
                  <TableCell>
                    <div className="text-low">{user.ReportName}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-low">{user.Category}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-low">{user.Revenue}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-low">
                      <div>{`${user.PlatformFee}`}</div>
                    </div>
                  </TableCell>
                  <TableCell>
