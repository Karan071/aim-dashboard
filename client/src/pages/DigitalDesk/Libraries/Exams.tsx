...
import {  ExamsTable } from "@/data/Data";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--text-head)]">Exams Desk</h1>
        <StatsCards />
   <Topbar  />
    ...
    <div className="flex justify-between px-4 py-3 bg-[var(--background)] rounded-sm gap-4 border flex-wrap shadow-none">
      ...
      <div className="flex gap-4 flex-wrap">
        <Button variant="standard" size="new">
          ...
          <span className=""> Exam Categories</span>
        </Button>
        <Button variant="standard" size="new">
...
function TableSection() {
  ...
  const [selectedStack, setSelectedStack] = useState<
    typeof ExamsTable
  >(ExamsTable[0] ? [ExamsTable[0]] : []);
  const [focusedId, setFocusedId] = useState<string | null>(ExamsTable[0] ? String(ExamsTable[0].id) : null);

  // Sorting logic
  const sortedData = [...ExamsTable];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
      let aValue = a[sortConfig.key as keyof typeof a];
      let bValue = b[sortConfig.key as keyof typeof b];
      // Special handling for 'for' (array), 'questions', 'responses', and 'lastUpdated' (date)
      if (sortConfig.key === "Created On") {
        // Parse as date
  ...
  const handleRowClick = (user: (typeof ExamsTable)[0]) => {
          ...
          <Table className="w-full caption-top border-collapse overflow-y-visible">
            <TableHeader className="bg-[var(--faded)] hover:bg-[var(--faded)] dark:bg-[var(--faded)] opacity-100">
              <TableRow>
                ...
                <TableHead
                  onClick={() => requestSort("Exam Name")}
                  className="cursor-pointer text-[var(--text)] text-low"
                >
                  Exam Name{" "}
                  {sortConfig?.key === "Exam Name" &&
                    (sortConfig.direction === "ascending" ? "↑" : "↓")}
                </TableHead>
                <TableHead
                ...
                <TableHead
                  onClick={() => requestSort("Created On")}
                  className="cursor-pointer text-[var(--text)]"
                >
                  Created On{" "}
                  {sortConfig?.key === "Created On" &&
                    (sortConfig.direction === "ascending" ? "↑" : "↓")}
                </TableHead>
              ...
              {currentRecords.map((user) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-4">
                      <div>
                        <div className="flex justify-start items-center">
                          <div className="font-medium">{user.ExamName}</div>
                        </div>
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                   ...
                   <TableCell>
                    <div className="text-low">{user["Created On"]}</div>
                  </TableCell>
                  <TableCell>
