...
function TableSection() {
  ...
  const [selectedStack, setSelectedStack] = useState<
    typeof CommentTable
  >(CommentTable[0] ? [CommentTable[0]] : []);
  const [focusedId, setFocusedId] = useState<string | null>(CommentTable[0]?.id || null);

  // Sorting logic
  const sortedData = [...CommentTable];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
  ...
  const handleRowClick = (user: (typeof CommentTable)[0]) => {
            ...
            <TableBody className="overflow-visible relative z-0">
              {currentRecords.map((user) => (
                ...
                >
                  ...
                  <TableCell>
                    <div className="text-low">{user.by}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-low">{user.on}</div>
                  </TableCell>
