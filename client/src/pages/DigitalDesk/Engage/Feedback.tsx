...
import { FeedbackTable } from "@/data/Data";
    ...
    <div className="flex justify-between px-4 py-3 bg-[var(--background)] rounded-sm gap-4 border flex-wrap shadow-none">
      ...
      <div className="flex gap-4 flex-wrap">
        <Button variant="standard" size="new">
          ...
          <span className="">  View with Screenshot</span>
        </Button>
        <Button variant="standard" size="new">
          ...
          <div className="p-6 overflow-y-auto relative w-full">
            {activeTab === "Category" && (
              <>
                ...
                <div className="flex flex-col gap-4 text-[var(--text)] ">
                  {["Tech", "Creative", "Communication", "Other"].map((option) => (
                    <label key={option} className="flex items-center gap-2">
                      <Checkbox
                        checked={categories.includes(option)}
                        onCheckedChange={() => handleCheckboxChange(setCategories, option)}
                      />
                      {option}
                    </label>
                  ))}
            ...
            {activeTab === "Target Segment" && (
              <>
                ...
                <div className="flex flex-col gap-4 text-[var(--text)] ">
                  {["K-12", "UG", "PG", "Professional"].map((option) => (
                    <label key={option} className="flex items-center gap-2">
                      <Checkbox
            ...
            {activeTab === "Tags" && (
              <>
                ...
                <div className="flex flex-col gap-4 text-[var(--text)] ">
                  {["Trending", "Future Skill"].map((option) => (
                    <label key={option} className="flex items-center gap-2">
                      <Checkbox
...
function TableSection() {
  ...
  >(FeedbackTable[0] ? [FeedbackTable[0]] : []);
  const [focusedId, setFocusedId] = useState<string | null>(FeedbackTable[0] ? String(FeedbackTable [0].id) : null);


  const sortedData = [...FeedbackTable];
  if (sortConfig !== null) {
  ...
  const handleRowClick = (user: (typeof FeedbackTable)[0]) => {
          ...
          <Table className="w-full caption-top border-collapse overflow-y-visible">
            <TableHeader className="bg-[var(--faded)] hover:bg-[var(--faded)] dark:bg-[var(--faded)] opacity-100">
              <TableRow>
                ...
                <TableHead
                  onClick={() => requestSort("submitted_for")}
                  className="cursor-pointer text-[var(--text)]"
                >
                  Submitted For{" "}
                  {sortConfig?.key === "submitted_for" &&
                    (sortConfig.direction === "ascending" ? "↑" : "↓")}
                </TableHead>
               
                <TableHead
                  onClick={() => requestSort("screenshot")}
                  className="cursor-pointer text-[var(--text)]"
                >
                 Screenshot {" "}
                  {sortConfig?.key === "screenshot" &&
                    (sortConfig.direction === "ascending" ? "↑" : "↓")}
                </TableHead>
              
                <TableHead className="text-[var(--text)]">Message</TableHead>
                <TableHead
                  onClick={() => requestSort("can_be_contacted")}
                  className="cursor-pointer text-[var(--text)]"
                >
                  Can be Contacted {" "}
                  {sortConfig?.key === "can_be_contacted" &&
                    (sortConfig.direction === "ascending" ? "↑" : "↓")}
                </TableHead>
            ...
            <TableBody className="overflow-visible relative z-0">
              {currentRecords.map((user) => (
                ...
                >
                  ...
                  <TableCell>
                    <div className="text-low">{user.submitted_on}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-low">{user.submitted_for}</div>
                  </TableCell>
                  
                  <TableCell>
                    <div className="text-low">{user.screenshot}</div>
                  </TableCell>
                
                  <TableCell>
                    <Badge variant="standard">{user.message}</Badge>
                  </TableCell>
                  <TableCell>
                    <Badge variant="standard">{user.can_be_contacted}</Badge>
                  </TableCell>
                  <TableCell>
