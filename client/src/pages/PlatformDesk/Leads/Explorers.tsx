...
function TableSection() {
  ...
  const [focusedId, setFocusedId] = useState<string | null>(LeadExplore[0]?.id || null);
  ...
  const bringToTop = (userId: string) => {
    const user = selectedStack.find((c) => c.id === userId);
    if (user) {
  ...
  useEffect(() => {
    ...
    allRows.forEach((row) => {
      ...
      const isInStack = selectedStack.some((user) => user.id === id);
