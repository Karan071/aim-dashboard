...
import {CoachLeadsTable, coachesList   } from "@/data/Data";
...
function TableSection() {
  ...
  const [coachesData, setCoachesData] = useState(CoachLeadsTable);

  // Sorting logic
  const sortedData = [...coachesData];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
  ...
  const handleAssignUsers = (userId: string) => {
    ...
    const user = coachesData.find(u => u.id === userId);
  ...
  const handleSaveAssignment = () => {
    if (currentUserForAssignment) {
      const updatedUsers = coachesData.map(user => {
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex justify-center items-center p-4">
          <div className="relative w-full max-w-[500px] rounded-sm bg-[var(--background)] border">
            ...
            <div className="p-6">
              ...
              {(() => {
                const currentUser = coachesData.find(u => u.id === currentUserForAssignment);
